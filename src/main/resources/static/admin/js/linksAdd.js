layui.config({
	...
	//创建一个编辑器
 	...
 	form.on("submit(addLinks)",function(data){
	 	...
	 	//显示、审核状态
 		var homePage = data.field.homePage=="on" ? "自有" : "",
 			subPage = data.field.subPage=="on" ? "第三方" : "";
        belong = '';
 		if(subPage == '' && homePage == ''){
            belong = "暂不展示";
 		}else if(homePage == ''){
            ...
            belong = homePage + '，' + subPage;
 		}

 		addLinks = '{"userName":"'+ $(".userName").val() +'",';  //用户名称
