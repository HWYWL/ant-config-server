layui.config({
	...
	//加载页面数据
	var usersData = '';
	$.get("/static/admin/json/usersList.json", function(data){
		usersData = data;
	...
	$(".search_btn").click(function(){
		...
		if($(".search_input").val() != ''){
            ...
            setTimeout(function(){
            	$.ajax({
					url : "/static/admin/json/usersList.json",
					type : "get",
					dataType : "json",
	...
	$("body").on("click",".users_edit",function(){  //编辑
		layer.alert('您点击了用户编辑按钮，由于是纯静态页面，所以暂时不存在编辑内容，后期会添加，敬请谅解。。。',{icon:6, title:'配置编辑'});
	})
