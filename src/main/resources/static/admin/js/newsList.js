layui.config({
	...
	$.get("/static/admin/json/newsList.json", function(data){
		var newArray = [];
		//单击首页“待审核配置”加载的信息
		if($(".top_tab li.layui-this cite",parent.document).text() == "待审核配置"){
	...
	$(".audit_btn").click(function(){
		...
		if($checkbox.is(":checked")){
            layer.confirm('确定审核通过？',{icon:3, title:'提示信息', btn: ['通过', '不通过', '取消']},function() {
                    ...
                    for (var j = 0; j < $checked.length; j++) {
                        for (var i = 0; i < newsData.length; i++) {
                            if (newsData[i].newsId == $checked.eq(j).parents("tr").find(".news_del").attr("data-id")) {
                                //修改列表中的文字
                                $checked.eq(j).parents("tr").find("td:eq(3)").text("审核成功").removeAttr("style");
                                //将选中状态删除
	...
	$("body").on("click",".news_edit",function(){  //编辑
		layer.alert('您点击了配置编辑按钮，由于是纯静态页面，所以暂时不存在编辑内容，后期会添加，敬请谅解。。。',{icon:6, title:'配置编辑'});
	})
